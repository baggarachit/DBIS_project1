...
app.get('/pointstable/:year', (req,res1) => {
  ...
  and season_year=2011 group by match.match_id,innings_no order by match_id)
  ...
  select team_name,mat,win,loss,tied,((runsscor*1.0/ballsscor)
  -(runsagainst*1.0/ballsagainst)) as nrr,pts from inf33 order by pts desc,nrr desc`;
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
