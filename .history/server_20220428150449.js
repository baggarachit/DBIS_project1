...
app.get('/get-new-exam', (req,res1) => {
  ...
  // console.log("owow");
  // console.log(ud);
  var string = `select DISTINCT Q.id from Question as Q, ques_subtopic as QS, subtopic_topic as ST where QS.q_id=Q.id and ST.st_id=QS.st_id and ST.t_id in ${topics_str} limit 10`;
  console.log(topics);
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
