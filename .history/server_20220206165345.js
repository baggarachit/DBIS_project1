      ...
      client.query(string,(err, res2) =>{
        if(!err){
          ...
          client.query(string,(err, res3)=>{
            if(!err){
              dic["data3"]=res3.rows;
              string=`with inf0 as (with inf(match_id,runs,balls,overs,wickets) as (select match_id,sum(runs_scored),sum(1),count(distinct over_id),sum(case when out_type is null then 0 else 1 end) from ball_by_ball where bowler=${playerid} group by match_id) select count(*) as matches,sum(runs) as runs,sum(balls) as balls,sum(overs) as overs,sum(wickets) as wickets,sum(case when wickets>=5 then 1 else 0 end) as five_wickets from inf) select *,runs*6.0/balls as economy from inf0`;
              client.query(string,(err,res4)=>{
                if(!err){
                  dic["data4"]=res4.rows;
                  string=`with inf(match_id,runs,wickets) as (select match_id,sum(runs_scored),sum(case when out_type is null then 0 else 1 end) from ball_by_ball where bowler=${playerid} group by match_id) select * from inf`;
