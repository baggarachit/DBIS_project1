...
app.get('/match/i1_pie/:matchID', (req,res1) => {
  var mid = req.params.matchID;
  var string = "with tmp(ones, twos, threes, fours, sixes, extras, total) as ( select sum((runs_scored=1)::int)*1.0, sum((runs_scored=2)::int)*2.0, sum((runs_scored=3)::int)*3.0, sum((runs_scored=4)::int)*4.0, sum((runs_scored=6)::int)*6.0, sum(extra_runs)*1.0, sum(runs_scored+extra_runs)*1.0 from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=1) select round(ones/total*100,2) as ones, round(twos/total*100,2) as twos, round(threes/total*100,2) as threes, round(fours/total*100,2) as fours, round(sixes/total*100,2) as sixes, round(extras/total*100,2) as extras from tmp";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
...
app.get('/match/i2_pie/:matchID', (req,res1) => {
  var mid = req.params.matchID;
  var string = "with tmp(ones, twos, threes, fours, sixes, extras, total) as ( select sum((runs_scored=1)::int)*1.0, sum((runs_scored=2)::int)*2.0, sum((runs_scored=3)::int)*3.0, sum((runs_scored=4)::int)*4.0, sum((runs_scored=6)::int)*6.0, sum(extra_runs)*1.0, sum(runs_scored+extra_runs)*1.0 from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=2) select round(ones/total*100,2) as ones, round(twos/total*100,2) as twos, round(threes/total*100,2) as threes, round(fours/total*100,2) as fours, round(sixes/total*100,2) as sixes, round(extras/total*100,2) as extras from tmp";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
