...
app.get('/venue/:venueID', (req,res1) => {
  ...
  var string = "with tmp(mid,inn,tr) as ( select M.match_id, B.innings_no, sum(B.runs_scored+B.extra_runs) from match as M, venue as V, ball_by_ball as B where M.venue_id = V.venue_id and V.venue_id=1 and B.match_id = M.match_id group by (V.venue_id, M.match_id, B.innings_no)) Select V.venue_name, V.city_name, V.country_name, V.capacity, count(DISTINCT M.match_id), (select max(tr) from tmp), (select min(tr) from tmp), coalesce((select max(T1.tr) from tmp as T1, tmp as T2 where T1.mid =T2.mid and T1.inn< T2.inn and T1.tr < T2.tr)+1,0) as highest_chased from venue as V, match as M, ball_by_ball as B where M.venue_id = V.venue_id and V.venue_id="+ and B.match_id = M.match_id group by V.venue_id";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
