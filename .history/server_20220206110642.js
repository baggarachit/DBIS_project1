...
app.get('/venue/:venueID', (req,res1) => {
  ...
  var string = "with tmp(mid,inn,tr) as ( select M.match_id, B.innings_no, sum(B.runs_scored+B.extra_runs) from match as M, venue as V, ball_by_ball as B where M.venue_id = V.venue_id and V.venue_id = "+String(vid)+" and B.match_id = M.match_id group by (V.venue_id, M.match_id, B.innings_no)) Select V.venue_name, V.city_name, V.country_name, V.capacity, count(DISTINCT M.match_id), (select max(tr) from tmp), (select min(tr) from tmp), coalesce((select max(T1.tr) from tmp as T1, tmp as T2 where T1.mid =T2.mid and T1.inn< T2.inn and T1.tr < T2.tr)+1,0) as highest_chased from venue as V, match as M, ball_by_ball as B where M.venue_id = V.venue_id and V.venue_id= "+String(vid)+" and B.match_id = M.match_id group by V.venue_id";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
...
app.post('/years',function(req,res1){

  var string="select DISTINCT season_year from match";
...
app.get('/pointstable/:year', (req,res1) => {
  ...
  var string=`with inf2 as (with inf1 as (with inf as (with i1(match_id,team1,team2,innings_no,runs,balls,match_winner,win_type,rr) as (with i0(match_id,team1,team2,innings_no,runs,balls,match_winner,win_type) as (select match.match_id,team1,team2,innings_no,sum(runs_scored+extra_runs),sum(1),match_winner,win_type from match,ball_by_ball where match.match_id=ball_by_ball.match_id and season_year=${year} group by match.match_id,innings_no order by match_id) select *,runs*1.0/balls from i0) select a.match_id,a.team1,a.team2,a.rr*6.0 as rr1,b.rr*6.0 as rr2,a.match_winner from i1 as a,i1 as b where a.innings_no!=b.innings_no and a.match_id=b.match_id and ((a.win_type='runs' and ((a.match_winner=a.team1 and a.innings_no=1) or (a.match_winner=a.team2 and a.innings_no=2))) or (a.win_type='wickets' and ((a.match_winner=a.team2 and a.innings_no=1) or (a.match_winner=a.team1 and a.innings_no=2))))) select match_id,team1,team2,rr1-rr2 as rr1,rr2-rr1 as rr2,1 as match1,1 as match2,case when match_winner=team1 then 1 else 0 end as team1win,case when match_winner=team2 then 1 else 0 end as team2win,match_winner from inf) select team_name,sum(1) as mat,sum(case when team_id=team1 then team1win else team2win end) as win,0 as tied,sum(case when team_id=team1 then rr1 else rr2 end) as NRR,sum(case when match_winner=team_id then 2 else 0 end) as pts from inf1,team where team1=team_id or team2=team_id group by team_name order by pts desc) select *,mat-win as loss from inf2`;
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
