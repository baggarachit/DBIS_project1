...
app.get('/getpaper/:difficulty/:duration/:marks/:topics/:c_id',function(req,res1){
  ...
  var string = `select DISTINCT Q.id, Q.difficulty, Q.time_taken from Question as Q, ques_subtopic as QS, subtopic_topic as ST, topic_course as TC where QS.q_id=Q.id and ST.st_id=QS.st_id and ST.t_id in ${topics_str} and TC.c_id=${cid} and ST.t_id=TC.t_id`;
  console.log(topics);
  client.query(string,(err, res) =>{
    if(!err){
      console.log(res.rows);
