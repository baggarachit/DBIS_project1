...
app.get('/courses/:uid', (req,res1) => {
  ...
  console.log(ud);
  // var pwd = req.params.pwd;
  var string = "select * from student_course as SC, courses as C where SC.c_id=C.id and SC.s_id = "+ud;
  console.log(string);
  client.query(string,(err, res) =>{
    if(!err){
      console.log("yaya");
...
app.get('/isfeed/:u_id/:q_id', (req,res1) => {
  ...
  var qd = req.params.q_id;
  // console.log(ud);
  var string = "select * from feedback as F where F.s_id = "+String(ud)+"F.q_id = "+String(qd);
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
