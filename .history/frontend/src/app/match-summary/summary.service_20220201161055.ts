...
export class SummaryService {
  ...
  getData_i1_pie(matchIdFromRoute: Number) : Observable<any>{
    let url="http://localhost:3080/match/i1_pie/"+String(matchIdFromRoute);
    return this.http.get(url);
  }
  getData_i2_pie(matchIdFromRoute: Number) : Observable<any>{
    let url="http://localhost:3080/match/i2_pie/"+String(matchIdFromRoute);
    return this.http.get(url);
  }
}
