...
app.get('/getpaper/:difficulty/:duration/:marks/:topics',function(req,res1){
  ...
  var string = `select DISTINCT Q.id, Q.difficulty, Q.time_taken from Question as Q, ques_subtopic as QS, subtopic_topic as ST, topic_course as TC where QS.q_id=Q.id and ST.st_id=QS.st_id and ST.t_id in ${topics_str} and TC.c_id=${course} and ST.t_id=TC.t_id`;
  console.log(topics);
  client.query(string,(err, res) =>{
    if(!err){
      var time=[];
        ...
        for(var j=-1;j<=1;j+=1){
          ...
          for(var ii = 0;ii<global.queslist.length;ii++){
            ...
            if(sumdiff==lis.length*req_diff){
              ...
              console.log(lis);
              for(var i=0;i<lis.length;i++) lis[i]=qid[lis[i]];
