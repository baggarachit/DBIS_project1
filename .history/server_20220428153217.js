...
app.get('/get-new-exam', (req,res1) => {
  ...
  for(var i=0;i<topics.length;i++) {
    ...
    if(isNaN(topics[i])) res1.send("Wrong sub-topics, please enter numbers");
  ...
  // console.log("owow");
  // console.log(ud);
  var string = `select DISTINCT Q.id from Question as Q, ques_subtopic as QS, subtopic_topic as ST, topic_course as TC where QS.q_id=Q.id and ST.st_id=QS.st_id and ST.t_id in ${topics_str} and TC.c_id=${course} and ST.t_id=TC.t_id limit 10`;
  console.log(topics);
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
