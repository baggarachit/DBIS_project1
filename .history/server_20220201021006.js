...
app.get('/match/inn/:matchID', (req,res1) => {
  var mid = req.params.matchID;
  var string1 = "select sum(B.runs_scored+B.extra_runs) over (order by (over_id, ball_id) asc rows between unbounded preceding and current row), (B.out_type is not NULL) as wicket from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=1";
  var string2 = "select sum(B.runs_scored+B.extra_runs) over (order by (over_id, ball_id) asc rows between unbounded preceding and current row), (B.out_type is not NULL) as wicket from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=1";
  var comb = []
  client.query(string1,(err, res) =>{
      comb.push(res.rows);
  });
  client.query(string2,(err, res) =>{
    comb.push(res.rows);
  });
  res1.send(comb);
});
...
app.get('/match/i1/:matchID', (req,res1) => {
  var mid = req.params.matchID;
  var string = "select sum(B.runs_scored+B.extra_runs) over (order by (over_id, ball_id) asc rows between unbounded preceding and current row), (B.out_type is not NULL) as wicket from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=1";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
...
app.get('/match/i2/:matchID', (req,res1) => {
  var mid = req.params.matchID;
  var string = "select sum(B.runs_scored+B.extra_runs) over (order by (over_id, ball_id) asc rows between unbounded preceding and current row), (B.out_type is not NULL) as wicket from ball_by_ball as B where B.match_id = "+String(mid)+" and B.innings_no=2";
  client.query(string,(err, res) =>{
    if(!err){
      res1.send(res.rows);
    } else{
